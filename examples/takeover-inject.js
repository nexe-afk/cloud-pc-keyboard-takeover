 ...
 *  6) 鼠标点击在桌面空白处可能【无可见变化】(正常现象), 别据此判"鼠标不通"。
 *    判"鼠标是否通"必须用可逆靶心: 点任务栏开始按钮(归一化约 0.02, 0.985)开菜单 -> Esc 关闭。
 *    实测 action 约 3256px(22.6% 像素) 而往返差仅 2px => 通路与可逆性双重成立。
 *  7) 右键/左键的载荷格式相同(均为 {button, preventDefault}), 都真实到达 pc.sendMousedown;
 *    右键在空白桌面无可见响应属远端行为, 不是注入失败。
 */
(function () {
  'use strict';
